...
int cant_minas() {
    int cant_minas;
    cant_minas = ((N*M)*10)/100;
    return cant_minas;
}

int cant_zombis() {
    int cant_zombis;
    cant_zombis = ((N*M)*5)/100;
    return cant_zombis;
}
...
void Mov_Zombis(char matrix[N][M] , pos_zombi_x , pos_zombi_y) {
...
//matriz de juego
char juego[N][M];
//cantidades de minas , zombis y teletraportadores 
int cantidad_minas = cant_minas();
int cantidad_zombis= cant_zombis();
int cantidad_tp = cant_tp();
