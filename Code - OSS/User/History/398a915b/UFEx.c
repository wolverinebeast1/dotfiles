...
matrix[p_x][p_y] = matrix[p_x+1][p_y];
...
matrix[p_x][p_y] = matrix[p_x][p_y-1];
...
if(matrix[p_x][p_y] == mina){
    matrix[p_x][p_y] == 'X';
        ...
        p_x =  N-1;
        p_y =  N/2;
        generate_mines(matrix, minas , p_x , p_y) ;
        }
...
Init(game);
generate_mines(game,mines , pos_x , pos_y);
